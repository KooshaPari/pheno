//! # phenotype-flags — canonical feature-flag set
//!
...
mod tests {
    ...
    fn test_from_env_invalid_value() {
        // Set an env var with invalid value, then attempt to load
        unsafe { std::env::set_var("TESTAPP_BADFLAG", "maybe") };
        let result = FlagSet::from_env("TESTAPP");
        unsafe { std::env::remove_var("TESTAPP_BADFLAG") };
        result.unwrap();
    }
