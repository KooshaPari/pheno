...
fn test_adapter_new_invalid_dir() {
    ...
    let adapter = GitVcsAdapter::new(dir.path().to_path_buf());
    assert!(git2::Repository::discover(adapter.repo_root()).is_err());
}
...
fn test_get_feature_history() {
    ...
    let history = get_feature_history(&adapter, "my-feature");
...
fn scan_all_features(adapter: &GitVcsAdapter) -> std::io::Result<Vec<String>> {
    let specs = adapter.repo_root().join("kitty-specs");
    if !specs.exists() {
        return Ok(vec![]);
    }
    ...
    for entry in std::fs::read_dir(specs)? {
        ...
        if meta.exists() {
            out.push(dir.file_name().to_string_lossy().to_string());
        }
    }
    Ok(out)
}
...
fn get_feature_history(adapter: &GitVcsAdapter, feature_slug: &str) -> Vec<FeatureHistoryEntry> {
    let output = Command::new("git")
        .args(["log", "--pretty=%s"])
        .current_dir(adapter.repo_root())
        .output()
        .expect("git log");

    let out = String::from_utf8_lossy(&output.stdout);
    out.lines()
        .filter(|line| line.contains(feature_slug))
        .map(|line| FeatureHistoryEntry {
