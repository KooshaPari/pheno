...
use thegent_memory::v2::{
    adapters::{GraphitiAdapter, HippoAdapter, ZepAdapter},
...
fn provider_from_str(s: &str) -> Result<MemoryProvider, c_int> {
    match s {
        ...
        "graphiti" => Ok(MemoryProvider::Graphiti),
        "hippo" => Ok(MemoryProvider::Hippo),
        "zep" => Ok(MemoryProvider::Zep),
...
pub extern "C" fn pheno_memory_new(provider: *const c_char) -> *mut c_void {
    ...
    let port: Arc<dyn MemoryPort> = match provider {
        ...
        MemoryProvider::Graphiti => Arc::new(GraphitiAdapter::default_endpoint()),
        MemoryProvider::Hippo => Arc::new(HippoAdapter::default_endpoint()),
        MemoryProvider::Zep => Arc::new(ZepAdapter::default_endpoint()),
