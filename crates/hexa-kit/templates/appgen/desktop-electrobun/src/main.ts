...
async function bootServices(): Promise<void> {
  if (!SERVICES_COMPOSE_FILE) {
    logger.info("SERVICES_COMPOSE_FILE not set — skipping service boot");
    return;
  }
  logger.info("Booting services: process-compose up -d");
  try {
    const result =
      await $`process-compose up -d --config ${SERVICES_COMPOSE_FILE}`.quiet();
    logger.info("Services:", result.text().trim());
  } catch (err) {
    logger.warn(
      "process-compose boot skipped (not found or services already running):",
...
function createMainWindow(): BrowserWindow {
  ...
  try {
    win.webview.executeJavascript(
      `window.__RENDERER_URL__ = ${JSON.stringify(DEV_URL)};`,
    );
  } catch {
...
async function main(): Promise<void> {
  ...
  setupMenu(win);
  logger.info(`Launched → ${DEV_URL} (fallback ${RENDERER_URL})`);
}
