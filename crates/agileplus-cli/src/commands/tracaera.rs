    ...
    S: StoragePort + EventStore,
{
    ...
    let graph = build_trace_graph(storage, feature).await?;
...
async fn build_trace_graph<S>(storage: &S, feature: Feature) -> Result<TracaeraTraceGraph>
where
    S: StoragePort + EventStore,
{
    let wps = storage
