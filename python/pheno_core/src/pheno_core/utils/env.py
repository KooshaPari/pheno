"""
Centralized environment variable access for pheno-sdk.
"""
...
def reload_env(dotenv_mapping: Mapping[str, str | None] | None = None) -> None:
    ...
    # Compute override flag from .env content first
    _FORCE_ENV_OVERRIDE = _compute_force_override(_DOTENV_VALUES)

    # Load .env to populate os.environ if available, honoring override semantics
    if load_dotenv is not None and _ENV_PATH.exists():
        try:
            load_dotenv(dotenv_path=_ENV_PATH, override=_FORCE_ENV_OVERRIDE)
        except TypeError:
            # Older python-dotenv versions may not support named args/override
